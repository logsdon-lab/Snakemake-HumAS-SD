...
def main():
    ...
    df_stringdecomposer = pl.read_csv(
        os.path.join(
            wd,
            "../data/HG01596_chr3_haplotype1-0000018:7760823-18212765_stringdecomposer.bed",
        ),
        separator="\t",
        new_columns=[
            "read-name",
    ...
    df_humas_hmmer = pl.read_csv(
        os.path.join(
            wd,
            "../data/HG01596_chr3_haplotype1-0000018:7760823-18212765_humas_hmmer.bed",
        ),
        separator="\t",
        new_columns=[
            "chrom",
        ...
        # Strand (-) -> Monomer'
        new_name=pl.col("name").cast(pl.String)
        + pl.when(pl.col("strand") == "-").then(pl.lit("'")).otherwise(pl.lit(""))
    )
    ...
    intervals_stringdecomposer = it.IntervalTree(
        it.Interval(*r)
        for r in df_stringdecomposer.select(
            "start-pos", "end-pos", "best-monomer"
        ).rows()
    )
